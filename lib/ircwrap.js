...
function getClient(userid, password, handlers, conn, cb) {
  ...
  if (typeof handlers !== 'object') return cb(new Error('onNewChan must be a function object'))
  ...
  couch.getAuthedUser(userid, password, function(err, user) {
    ...
    if (clients.hasOwnProperty(userid))
      return cb(null, usingClient(wrapClient(clients[userid], user, conn), conn, false, handlers))
    makeClient(user, function(err, client) {
      if (err) return cb(err)
      clients[userid] = client
      client = wrapClient(client, user, conn)
      usingClient(client, conn, true, handlers)
      cb(null, client)
    })
  })
}
...
function usingClient(wrapper, conn, firstUse, handlers) { // FIXME onNewChan -> handlers
  conn.user = wrapper.user
  ...
  if (handlers.onNewChan) {
    handlers.onNewChan.forEach(function(handler) {
      Object.keys(wrapper.client.chans).forEach(handler)
    })
    wrapper.on('join', function(chan, who) {
      if (who === client.nick) {
        handlers.onNewChan.forEach(function(handler) {
          handler(chan)
        })
  ...
  ['message', 'selfMessage', 'nick', 'topic', 'join', 'part', 'kick', 'quit', 'kill', 'names', 'topic'].forEach(function(eventName) {
...
function wrapClient(client, user, conn) {
  ...
  Object.defineProperty(wrapper, 'user', {value: user})
