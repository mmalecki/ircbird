...
function getClient(userid, password, onNewChan, conn, cb) {
  ...
  if (typeof onNewChan !== 'function') return cb(new Error('onNewChan must be a function'))
  ...
  couch.getAuthedUser(userid, password, function(err, user) {
    ...
    if (clients.hasOwnProperty(userid))
      return cb(null, usingClient(wrapClient(clients[userid], user, conn), conn, false, onNewChan))
    makeClient(user, function(err, client) {
      if (err) return cb(err)
      clients[userid] = client
      client = wrapClient(client, user, conn)
      usingClient(client, conn, true, onNewChan)
      cb(null, client)
    })
  })
}
...
function usingClient(wrapper, conn, firstUse, onNewChan) {
  ...
  Object.keys(wrapper.client.chans).forEach(onNewChan)
