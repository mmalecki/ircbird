...
exports.fetchLastChatlines = function(user, network, channel, cb) {
  exports.fetchChatlines(user, ['ircbird.log', network, channel, 'Z'].join(':'), true, cb)
}
...
function _fetchChatlines(user, docid, backwards, cb) {
  ...
  db.query('GET', '_all_docs', options, function(err, result) {
    if (err) return cb(err)
    console.log('_fetchChatlines got '+result.length+' raw rows')
    result = result.map(function(a){return a}) // this is because couch is FUCKING STUPID
    ...
    console.log('_fetchChatlines got '+result.length+' rows')
